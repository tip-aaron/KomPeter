...
public class ProductOperations {
    ...
    public static void addProduct(final String name, final String description, final ProductCategory category,
            final ProductBrand brand, final String displayImage, final int minimumQuantity,
        ...
        try (Connection conn = factory.getConnection()) {
            final int productId = productDao.createProduct(conn,
                    Product.builder().id(-1).markupRate(markupRate).minimumQuantity(minimumQuantity).name(name)
                            .description(description).displayImage(displayImage).category(category).brand(brand)
                            .build());
