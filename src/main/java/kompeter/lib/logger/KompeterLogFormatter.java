...
public class KompeterLogFormatter extends Formatter {
    ...
    private void appendException(final StringBuilder sb, final Throwable err) {
        ...
        final String simpleName = err.getClass().getCanonicalName();

        sb.append(simpleName).append(": ").append(err.getMessage());

        sb.append("\n");

        final StackTraceElement[] trace = err.getStackTrace();

        for (final StackTraceElement element : trace) {
            sb.append("\t\tat ").append(element).append("\n");
        }

        final Set<Throwable> circularReferenceSet = Collections.newSetFromMap(new IdentityHashMap<>());
        final Throwable[] suppressed = err.getSuppressed();

        if (suppressed.length != 0) {
            sb.append("\tSuppressed: ").append("\n");

            for (final Throwable se : suppressed) {
    ...
    private void printEnclosedStackTrace(final StringBuilder sb, final Throwable err,
        ...
        final int m = trace.length - 1;
        final int n = enclosingTrace.length - 1;
        final int framesInCommon = computeFramesInCommon(trace, enclosingTrace, m, n);
        ...
        if (cause != null) {
            ...
            printEnclosedStackTrace(sb, cause, trace, "\tCaused by: ", prefix + "\t", circularReferenceSet);
        }
    }
    ...
    public String format(final LogRecord record) {
        ...
        stringBuilder.append(formateDateString(record.getMillis())).append(" - ").append(record.getSourceClassName())
                .append(".").append(record.getSourceMethodName()).append(" - ").append(record.getLevel().getName())
                .append(" - ").append(formatMessage(record));

        stringBuilder.append("\n");

        final Throwable errThrowable = record.getThrown();
